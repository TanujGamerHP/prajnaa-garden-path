...
import { MessageSquare, Sparkles, Send, X, Loader2, User, HelpCircle } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { askPrajnaaCopilot, ChatMessage } from "@/lib/api/copilot-service";

export function PrajnaaCopilot() {
  const { user } = useAuth();
  ...
  // Check if current page is in the forbidden portals
  const isForbiddenPath =
    location.pathname.startsWith("/admin") ||
    location.pathname.startsWith("/farmer-portal");

  // Show a welcome message when user opens the chat for the first time
  useEffect(() => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      // Gather only user and model messages for history (filter out system/error messages)
      const chatHistory = messages.filter((m) => m.role !== "system");

      const response = await askPrajnaaCopilot(text, user?.id, chatHistory);
  ...
  const suggestions = [
    { label: "Recommend immunity products", text: "Recommend some products for immunity" },
    { label: "Become farmer partner", text: "How can I sell products here and become a farmer partner?" },
    { label: "Track my order", text: "How do I track my order?" },
    { label: "Refund policy", text: "What is the return and refund policy?" }
  ];
    ...
    <>
      ...
      >
        {isOpen ? (
          ...
          <div className="relative flex items-center justify-center">
            ...
            <Sparkles className="h-3 w-3 absolute -top-1.5 -right-1.5 text-accent animate-pulse" />
            {unreadCount > 0 && (
              <span className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-accent text-[10px] font-bold text-accent-foreground animate-bounce">
                {unreadCount}
              </span>
            )}
          </div>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] h-[550px] max-h-[80vh] flex flex-col rounded-3xl bg-background border border-border shadow-2xl overflow-hidden animate-scale-up">
          ...
          {/* Header */}
          <div className="flex items-center justify-between bg-gradient-to-r from-primary/95 to-primary p-4 text-primary-foreground border-b border-border/10">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-background/15 border border-white/20">
            ...
            <button
              ...
              className="text-primary-foreground/85 hover:text-white rounded-full p-1 bg-white/10 hover:bg-white/20 transition cursor-pointer"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-secondary/10">
            {messages.map((m, idx) => (
              ...
              >
                ...
                >
                  {m.role === "user" ? <User className="h-3.5 w-3.5" /> : <Sparkles className="h-3.5 w-3.5 text-accent fill-accent" />}
                </div>
            ...
            {isLoading && (
              <div className="flex gap-2.5 max-w-[85%] mr-auto">
                <div className="h-7 w-7 rounded-full flex items-center justify-center shrink-0 border bg-primary/10 border-primary/20 text-primary">
                  <Sparkles className="h-3.5 w-3.5 text-accent fill-accent animate-spin" />
                </div>
                <div className="rounded-2xl p-3 shadow-sm bg-card border border-border/80 text-foreground rounded-tl-none flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground font-medium">Retrieving knowledge...</span>
                </div>
              </div>
            )}
          ...
          {inputValue.trim() === "" && !isLoading && (
            <div className="px-4 py-2 border-t border-border/40 bg-secondary/5 overflow-x-auto flex gap-2 no-scrollbar">
              {suggestions.map((s, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSendMessage(s.text)}
                  className="shrink-0 text-[10px] font-semibold bg-card border border-border hover:border-primary px-3 py-1.5 rounded-full hover:bg-secondary/40 text-foreground/80 cursor-pointer transition-all duration-200"
                >
                  {s.label}
                </button>
              ))}
          ...
          <div className="p-3 border-t border-border bg-background flex items-center gap-2">
            <input
              type="text"
              placeholder="Ask about organic products, farmers, orders..."
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              onKeyDown={handleKeyPress}
              disabled={isLoading}
              className="flex-1 font-subhead h-10 px-3 rounded-full border border-border focus:border-primary text-xs outline-none bg-secondary/25 focus:bg-background transition-all disabled:opacity-50"
            />
            <button
              onClick={() => handleSendMessage(inputValue)}
              disabled={isLoading || !inputValue.trim()}
              className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-primary-foreground hover:opacity-90 disabled:opacity-40 transition cursor-pointer"
            >
