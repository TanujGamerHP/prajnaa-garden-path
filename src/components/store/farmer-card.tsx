    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-foreground/60 to-transparent p-5 text-background">
          <p className="font-subhead text-[11px] uppercase tracking-[0.16em] opacity-80">
            {farmer.village}, {farmer.state}
          </p>
          <h3 className="font-display mt-1 text-2xl font-semibold">{farmer.name}</h3>
          <p className="mt-2 line-clamp-2 max-w-xs text-sm opacity-90">{farmer.storyPreview}</p>
        </div>
      </div>
    </Link>
  );
