  ...
  notFoundComponent: () => (
    <MarketingLayout>
      <div className="container-prj py-32 text-center">
        <p className="font-subhead text-xs uppercase tracking-[0.18em] text-muted-foreground">404</p>
    ...
    <MarketingLayout>
      <article>
        <header className="container-prj pt-16 md:pt-24">
          <Reveal inline>
            <Link to="/blog" className="font-subhead inline-flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-muted-foreground hover:text-foreground">
              <ArrowLeft className="h-3.5 w-3.5" /> The Journal
            </Link>
            <p className="font-subhead mt-6 text-[11px] uppercase tracking-[0.18em] text-accent">{post.category}</p>
            <h1 className="font-display mt-3 max-w-3xl text-4xl font-semibold leading-[1.05] md:text-6xl">{post.title}</h1>
            <p className="font-subhead mt-5 text-xs uppercase tracking-[0.14em] text-muted-foreground">
              {post.author} · {new Date(post.date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })} · {post.readMinutes} min read
            </p>
          </Reveal>
        </header>
            ...
            <Reveal key={p.slug} delay={i * 80}>
              <Link to="/blog/$slug" params={{ slug: p.slug }} className="group flex h-full flex-col">
                <div className="aspect-[4/3] overflow-hidden rounded-2xl bg-secondary">
                  <img src={p.image} alt={p.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.04]" />
                </div>
                <h3 className="font-display mt-4 text-lg font-medium">{p.title}</h3>
                <p className="font-subhead mt-1 text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{p.category}</p>
              </Link>
            </Reveal>
          ))}
        </div>
