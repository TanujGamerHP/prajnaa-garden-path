...
const features = [
  { outlet: "Vogue India", date: "May 2026", headline: "How Prajnaa is rewriting the supply chain for Indian pantry staples", url: "#" },
  { outlet: "Forbes India", date: "Apr 2026", headline: "The farmer-first marketplace that pays growers 4× the going rate", url: "#" },
  { outlet: "Mint Lounge", date: "Mar 2026", headline: "From a Himalayan field to your kitchen, with the batch code to prove it", url: "#" },
  { outlet: "YourStory", date: "Feb 2026", headline: "Inside Prajnaa's 4-step farmer onboarding — and why it works", url: "#" },
  { outlet: "The Hindu", date: "Jan 2026", headline: "A traceable jar of pickle, and the small farms behind it", url: "#" },
  { outlet: "Conde Nast Traveller", date: "Dec 2025", headline: "Eight Indian regions, eight harvests — a year in pantry travel", url: "#" },
];
...
export const Route = createFileRoute("/press")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Press features, awards, and brand resources for Prajnaa Farm." },
...
function PressPage() {
  return (
    <MarketingLayout>
      <PageHero
        eyebrow="Press"
        title={<>In their <span className="text-primary">words.</span></>}
        ...
        <ul className="mt-8 divide-y divide-border rounded-3xl border border-border bg-background">
          {features.map((f, i) => (
            <Reveal as="li" key={f.headline} delay={i * 40}>
              <a href={f.url} className="group flex items-start justify-between gap-6 p-6 transition-colors hover:bg-secondary/30 md:p-7">
                <div>
                  <p className="font-subhead text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{f.outlet} · {f.date}</p>
          ...
          {awards.map((a, i) => (
            <Reveal key={a.t} delay={i * 70}>
              <div className="h-full rounded-2xl border border-border bg-background p-6">
                ...
                <p className="font-subhead mt-2 text-xs uppercase tracking-[0.14em] text-muted-foreground">{a.who}</p>
              </div>
            </Reveal>
          ))}
        </div>
