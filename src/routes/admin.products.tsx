...
function AdminProducts() {
  ...
  const filtered = products.filter((p: any) => {
    ...
    return (
      ...
      p.farmer?.full_name?.toLowerCase().includes(s)
    );
  });
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {products.length} listings across all farmers.
          </p>
        </div>
            ...
            <tbody>
              {filtered.map((p: any) => (
                <tr key={p.id} className="border-t border-border">
                  ...
                  <td>
                    <span
                      className={`font-subhead rounded-full px-2 py-0.5 text-[11px] capitalize ${
                        p.status === "published"
                    ...
                    <div className="inline-flex gap-1">
                      {p.status !== "published" && (
                        <button
                          ...
                          className="font-subhead rounded-full border border-border px-2.5 py-1 text-[11px] hover:bg-secondary"
                        >
                          Publish
                        </button>
                      )}
                      ...
                      <button
                        onClick={() => remove(p.id)}
                        className="font-subhead rounded-full border border-destructive/30 px-2.5 py-1 text-[11px] text-destructive hover:bg-destructive/10"
                      >
                        Delete
                      </button>
                    </div>
